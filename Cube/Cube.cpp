...
Cube solve()
{
    ...
    while (!s.empty())
    {
        ...
        {
            cout << cube.count;
            cout << str321(cube.matrix).c_str() << endl;
        }

        if (check(cube.matrix))
        {
            cout << cube.count << '\n';
            for (int i = 0; i < 10; i++)
            {
                if (cube.path[i] != 0)
                    printf("%d ", cube.path[i]);
                else
                    return;
            }
            return;
        }
        string x = str321(cube.matrix);
        // cout << x.c_str() << endl;
        if (book.count(x) >= 1)
        {
            continue;
        ...
        else
        {
            ...
            for (int i = 1; i <= 18; i++)
            {
                *move = action_list(i, cube);
                (*move).path[cube.count] = i;
                if (cube.path[cube.count - 1] == cube.path[cube.count - 2] && i == cube.path[cube.count - 1])
                    continue;
                // 如果t.path数组中倒数第二个元素与倒数第一个元素相等，并且当前循环变量i等于倒数第一个元素，则跳过本次循环的剩余部分

                if ((i & 1) && cube.path[cube.count - 1] != i - 1)
                    s.push(*move);
                // 如果当前循环变量i是奇数，并且t.path数组中倒数第一个元素不等于i-1，则将action对象压入队列q中
