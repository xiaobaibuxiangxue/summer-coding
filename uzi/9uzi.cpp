...
things count1;
int minvalue(int x, int y, int alpha, int beta, int depth, int);
int maxvalue(int x, int y, int alpha, int beta, int depth, int);
void get() // 读取输入信息
{
    cin >> mycolor;
...
int maxvalue(int x, int y, int alpha, int beta, int depth, int va)
{
    int count;
    chess[x][y] = mycolor;
    if (depth <= 0)
    {
        int temp = va + evalue(x, y, mycolor);
        // printchess();
        // printf("(%d,%d,,%d,%d)", x, y, mycolor, temp);
        chess[x][y] = -1;
        return temp;
    }
    int v = -INF;
    for (int i = 0; i < 15; i++)
    {
        for (int j = 0; j < 15; j++)
        {
            if (chess[i][j] == -1)
            {
                ...
                v = max(v, minvalue(i, j, alpha, beta, depth - 1, va));
                // printf("|%d|", v);
                chess[i][j] = -1;
                // if (v >= beta)
                {
...
int minvalue(int x, int y, int alpha, int beta, int depth, int va)
{
    chess[x][y] = enemycolor;
    if (depth <= 0)
    {
        int temp = va - evalue(x, y, enemycolor);
        chess[x][y] == -1;
        return temp;
    }
    int v = +INF;
    for (int i = 0; i < 15; i++)
    {
        for (int j = 0; j < 15; j++)
        {
            ...
            {
                chess[i][j] = mycolor;
                v = min(v, maxvalue(i, j, alpha, beta, depth - 1, va));
                // cout << v << ' ';
                chess[i][j] = -1;
                // if (v <= alpha)
                {
...
void search()
{
    ...
    for (int i = 0; i < 15; i++)
    {
        for (int j = 0; j < 15; j++)
        {
            if (chess[i][j] == -1)
            {
                int temp = maxvalue(i, j, -INF, +INF, Depth, 0);
                // printf("!%d!", temp);
                if (maxv1 < temp)
                {
                    maxv1 = temp;
